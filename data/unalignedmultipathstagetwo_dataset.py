...
class UnalignedMultipathStageTwoDataset(BaseDataset):
    """
    ...
    def __init__(self, opt):
        ...
        self.b_df = pd.read_csv(os.path.join(df_path, 'df_b.csv'))
        self.c_df = pd.read_csv(os.path.join(df_path, 'df_c.csv'))
        self.d_df = pd.read_csv(os.path.join(df_path, 'df_d.csv'))
        ...
        self.b_image = sorted(self.b_df['image'].tolist())
        self.b_mask = sorted(self.b_df['mask'].tolist())
        self.c_image = sorted(self.c_df['image'].tolist())
        self.c_mask = sorted(self.c_df['mask'].tolist())
        self.d_image = sorted(self.d_df['image'].tolist())
        self.d_mask = sorted(self.d_df['mask'].tolist())
        ...
        self.b_size = len(self.b_image)
        self.c_size = len(self.c_image)
        self.d_size = len(self.d_image)
    ...
    def __getitem__(self, index):
        ...
        c_index = random.randint(0, self.c_size - 1)
        d_index = random.randint(0, self.d_size - 1)
        ...
        b_image_path = self.b_image[b_index]
        b_mask_path = self.b_mask[b_index]
        c_image_path = self.c_image[c_index]
        c_mask_path = self.c_mask[c_index]
        d_image_path = self.d_image[d_index]
        d_mask_path = self.d_mask[d_index]
        ...
        b_image, b_mask = self.mask_and_normalize(b_image_path, b_mask_path)
        c_image, c_mask = self.mask_and_normalize(c_image_path, c_mask_path)
        d_image, d_mask = self.mask_and_normalize(d_image_path, d_mask_path)
        ...
        return {
            ...
            'B_image': b_image, 'B_mask': b_mask, 'C_image': c_image, 'C_mask': c_mask, 'D_image': d_image, 'D_mask': d_mask
        }
